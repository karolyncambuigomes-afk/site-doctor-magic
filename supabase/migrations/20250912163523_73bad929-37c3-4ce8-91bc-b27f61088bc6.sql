...
-- Add encrypted columns to model_applications table first
...
CREATE OR REPLACE FUNCTION public.encrypt_sensitive_field(plain_text text, encryption_key text DEFAULT 'app_secret_key_2025_v1')
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.decrypt_sensitive_field(encrypted_text text, encryption_key text DEFAULT 'app_secret_key_2025_v1')
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
SELECT 
  ...
  -- Other sensitive fields - only accessible to admins
  ...
  CASE 
    ...
    ELSE ARRAY[COALESCE(array_length(photos, 1), 0)::text || ' photos (admin access required)']
  END as photos,
  
  CASE 
    WHEN public.is_admin() THEN videos  
    ELSE ARRAY[COALESCE(array_length(videos, 1), 0)::text || ' videos (admin access required)']
  END as videos,
