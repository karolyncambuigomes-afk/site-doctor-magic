-- Fix existing data and add RBAC constraints safely
-- 1. First update existing data to use new role system
UPDATE profiles SET role = 'member' WHERE role = 'user';

-- 2. Add constraint after data is cleaned
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;
ALTER TABLE profiles ADD CONSTRAINT profiles_role_check CHECK (role IN ('admin', 'team', 'member'));

-- 3. Create enhanced permission checking functions
...
-- 5. Create audit logging function for admin logins
